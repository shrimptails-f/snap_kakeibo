...
2. 認証と解析依頼を feature に分離する(完了: 認証は `features/auth`、アップロードは `/upload`、解析履歴は受付月・状態絞り込みとページネーションを含む `/analysis-requests`、支出詳細・編集は `features/expenses` の `/expenses/:expenseId`)
