...
var (
	...
	retry application.RetryAnalysisUsecaseInterface
	log   logger.Interface
)

func init() {
	osw := oswrapper.New()
	cfg, err := settings.LoadRetryAnalysis(osw)
	if err != nil {
		panic(err)
	}
	...
	container, err := di.NewRetryAnalysisContainer(cfg, awsCfg, osw, configuredLogger)
	if err != nil {
		panic(err)
	}
	...
	retry, err = di.ResolveRetryAnalysisUsecase(container)
	if err != nil {
		panic(err)
	}
}
...
func handler(ctx context.Context, req events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	requestID := req.PathParameters["analysisRequestId"]
	if requestID == "" {
		return apigateway.Error(400, "analysisRequestId path parameter is required")
	}
	out, err := retry.Retry(ctx, application.RetryAnalysisInput{UserID: user.UserID, AnalysisRequestID: requestID})
	if err != nil {
		switch {
		case errors.Is(err, application.ErrInvalidInput):
			return apigateway.Error(400, "invalid retry analysis request")
		case errors.Is(err, application.ErrAnalysisRequestNotRetryable):
			return apigateway.Error(409, "analysis request cannot be retried")
		}
		return events.APIGatewayV2HTTPResponse{StatusCode: 500}, err
	}
	return apigateway.JSON(200, response{AnalysisRequestID: out.AnalysisRequestID, Status: string(out.Status), Attempt: out.Attempt})
}
