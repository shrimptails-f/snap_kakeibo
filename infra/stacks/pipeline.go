...
func NewPipelineStack(scope constructs.Construct, id string, props *PipelineStackProps) *PipelineStack {
	...
	s.newServicePipeline("backend", props.Config, connectionARN, backendBuildSpec())
	s.newServicePipeline("frontend", props.Config, connectionARN, frontendBuildSpec())
	return s
}
...
func (s *PipelineStack) newServicePipeline(name string, cfg config.Config, connectionARN *string, buildSpec awscodebuild.BuildSpec) {
	...
	project := awscodebuild.NewPipelineProject(s.Stack, jsii.String(constructID(name)+"BuildProject"), &awscodebuild.PipelineProjectProps{
		ProjectName: jsii.String(cfg.ResourceName(name + "-cd")),
	...
	return awscodebuild.BuildSpec_FromObject(&map[string]any{
		...
		"phases": map[string]any{
			...
			"build": map[string]any{
				"commands": []string{"bash scripts/backend-cd.sh"},
			},
		},
	})
}
...
func frontendBuildSpec() awscodebuild.BuildSpec {
	return awscodebuild.BuildSpec_FromObject(&map[string]any{
		...
		"phases": map[string]any{
			...
			"build": map[string]any{
				"commands": []string{"bash scripts/frontend-cd.sh"},
			},
		},
	})
}
	...
	if name == "backend" {
		project.AddToRolePolicy(awsiam.NewPolicyStatement(&awsiam.PolicyStatementProps{
			Actions: jsii.Strings(
				...
				"lambda:GetAlias", "lambda:UpdateFunctionCode",
