...
TextractのconfidenceはOCR時点の値であり、ユーザーが商品名や金額を修正した後の明細には意味が薄くなるため。
...
Textractの解析結果をあとから検証したい場合は、明細にconfidenceを散らすのではなく、S3に解析結果JSONを保存し、`upload_histories.raw_textract_s3_key` から参照する。
...
| S3アップロード完了を処理する | StartTextract Lambda | user_id + upload_idで履歴を更新 | upload_histories | UpdateItem |
| Textract完了を処理する | ResultHandler Lambda | Textract通知のJobTagからuser_id + upload_id + attemptを復元 | upload_histories / billings / billing_details / monthly_summaries | TransactWriteItems |
| 解析失敗を記録する | StartTextract / ResultHandler Lambda | user_id + upload_idで履歴を更新 | upload_histories | UpdateItem |
...
カテゴリ別集計は `category_totals` にmapで持つ。

Textractの解析精度を考慮し、月の総額とカテゴリ別内訳は信頼度を分けて扱う。

月の総額は請求単位の `billings.final_amount` を正とする。

カテゴリ別内訳は `billing_details` の明細を元にする。

Textract由来のカテゴリは参考値として扱い、ユーザーが修正した明細では修正後の `category` を優先する。
...
```jsonc
{
  ...
  "category_totals": { // カテゴリ別金額。円グラフや積み上げ棒グラフの内訳に使う
    "food": 86000, // 食費カテゴリの合計金額
    "daily_goods": 22500, // 日用品カテゴリの合計金額
    "other": 20000, // その他カテゴリの合計金額
    "unknown": 12000 // 分類できなかった明細の合計金額
  },

  "version": 26, // 楽観ロック用。ResultHandlerはADD、再計算はCondition付きSET
...
主キーは、S3イベントやTextract完了処理から `user_id + upload_id` で直接更新できる形にする。
...
```jsonc
{
  ...
  "attempt": 1, // 解析の試行回数。再実行のたびに+1。ClientRequestTokenに使う
  "s3_key": "receipts/01JUSERXXX/01JUPLOADXXX/original.jpg", // 元画像を保存したS3キー
  "textract_job_id": "xxxxxxxx", // Textractの非同期ジョブID
  "raw_textract_s3_key": "textract-results/01JUSERXXX/01JUPLOADXXX/result.json", // Textract解析結果JSONを保存したS3キー
...
`NO_DATA` の場合も `upload_histories` は残す。Textract解析は完了したが明細0件のため、`billings` / `billing_details` / `monthly_summaries` は作成・更新しない。`error_code` は使わず、履歴画面では「登録対象なし」として表示する。
...
```jsonc
{
  ...
  "source": "TEXTRACT", // データの作成元。TEXTRACTまたはMANUAL
...
`textract_job_id` 用GSIも作らない。Textract開始時に `JobTag` へ `user_id`、`upload_id`、`attempt` を入れ、完了通知から対象のupload_historiesと試行回数を特定する。
...
| 商品件数 | 1ユーザー + 1月で1値 | `monthly_summaries.detail_count` | `billing_details` | Textractで取れた明細数 |
| カテゴリ別金額 | 1ユーザー + 1月のmap | `monthly_summaries.category_totals` | `billing_details.category` + `billing_details.amount` | 円グラフと積み上げ棒グラフの内訳 |

月合計はカテゴリ別金額の合計から作らない。

理由は、Textractでは請求合計より商品明細のほうが欠落・誤読・分割ミスが起きやすいため。
...
ユーザーがカテゴリを修正した場合は、月次再計算で `category_totals` を作り直す。
...
`ResultHandler Lambda` はTextract結果の検証後、OpenAI APIで明細カテゴリを分類してから、以下を `TransactWriteItems` で処理する。
...
3. billing_details (最大50件)
   Put
   category / category_source / ai_category_confidence を含める
...
二重処理と古いTextract通知の反映を防ぐため、upload_histories側に条件を置く。
...
AI分類に失敗した場合でも、Textract結果が有効なら請求と明細は作成する。分類できなかった明細は `category = unknown`、`category_source = UNKNOWN` として登録する。
...
upload_histories
  ...
  status = ANALYZING
  attempt = attempt + 1
  REMOVE textract_job_id, error_code, error_message, failed_at
```
...
4. monthly_summaries更新
   ...
   SET category_totals = SUM(billing_details.amount) GROUP BY category
   SET type / user_id / year_month
   SET version         = if_not_exists(version, 0) + 1
```

条件失敗した場合(再計算中にResultHandlerが `ADD` した場合)は 1 からやり直す。
