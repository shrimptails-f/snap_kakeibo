...
| 項目 | 値 |
| --- | --- |
| Analyze Lambda タイムアウト | 11分 |
| OpenAI 呼び出しの各試行 | 最大5分。1秒待って1回だけリトライ |
| OpenAI 呼び出しの合計上限 | 10分30秒。2回の試行と待機を含む |
| リトライ対象 | 429 / 500 / 502 / 503 / 504 / ネットワーク一時エラー |
| リトライしない | 400系の恒久エラー、JSON Schema 不一致 |

10分30秒以内に成功しなければエラーを返して SQS のリトライに任せる。`maxReceiveCount` 回失敗すると DLQ に入り、メールで通知される。恒久エラーは `FAILED`(`ANALYSIS_FAILED`)を記録して正常終了する。
...
| 可視性タイムアウト | Lambdaタイムアウトの6倍 | AWS推奨。Lambda 11分なら66分 |
